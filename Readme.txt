                            ...
                            <table class="w-full text-sm text-left" id="habitual-table">
                                <thead class="text-xs uppercase" style="background-color: var(--bg-secondary);">
                                    <tr>
                                        ...
                                        <th class="px-4 py-3">Tanggal Terlambat</th>
                                        <th class="px-4 py-3 text-center">Total Keterlambatan</th>
                                    </tr>
                                </thead>
...
const app = {
    ...
    init() {
        ...
        if (GOOGLE_SHEETS_API_KEY.includes('GANTI_DENGAN') || SPREADSHEET_ID.includes('GANTI_DENGAN')) {
            ...
            this.utils.setDefaultDates(); // Set default dates before loading data
            this.loadData();
        }
    ...
    async loadData() {
        ...
        try {
            ...
            this.state.data.all = rows.slice(1).map(row => {
                const obj = {};
                this.state.headers.forEach((h, i) => {
                    let value = row[i] || '';
                    // Trim student names to ensure consistency
                    if (h.toLowerCase() === 'nama' && typeof value === 'string') {
                        value = value.trim();
                    }
                    obj[h.toLowerCase()] = value;
                });
                return obj;
            }).filter(row => row.tanggal && row.nama); // Ensure essential data exists

            this.state.data.filtered = [...this.state.data.all];
            this.renderAll();
            this.handleHabitualDetection(); // Run detection with default dates
    ...
    handleHabitualDetection() {
        ...
        // 1. Filter all records by the selected date range.
        ...
        // 2. Count occurrences for each student.
        const counts = filteredByDate.reduce((acc, row) => {
            const studentName = row.nama.trim(); // Use trimmed name for accuracy
            if (!acc[studentName]) {
                acc[studentName] = { count: 0, kelas: row.kelas };
            }
            acc[studentName].count++;
            return acc;
        }, {});
        
        // 3. Get the names of students who meet the minCount criteria.
        const habitualOffenderNames = Object.keys(counts).filter(name => counts[name].count > minCount);

        // 4. Create the final results list with individual records for the offenders.
        const detailedResults = filteredByDate
            .filter(row => habitualOffenderNames.includes(row.nama.trim()))
            .map(row => ({
                ...row, // a single late record, contains 'nama', 'kelas', 'tanggal', etc.
                totalCount: counts[row.nama.trim()].count // add the total count for that student
            }));

        // 5. Sort the results to group by student, then by date
        detailedResults.sort((a, b) => {
            if (a.nama < b.nama) return -1;
            if (a.nama > b.nama) return 1;
            const dateA = this.utils.parseDate(a.tanggal);
            const dateB = this.utils.parseDate(b.tanggal);
            return dateA && dateB ? dateA - dateB : 0;
        });

        this.state.habitualResults = detailedResults;
    ...
    renderHabitualTable() {
        ...
        container.innerHTML = results.map(record => `
            <tr class="border-b transition-colors" style="border-color: var(--border-color);">
                <td class="px-4 py-3 font-semibold">${record.nama}</td>
                <td class="px-4 py-3">${record.kelas}</td>
                <td class="px-4 py-3">${record.tanggal}</td>
                <td class="px-4 py-3 text-center font-bold text-lg">${record.totalCount}</td>
            </tr>
        `).join('');
    },
    ...
    handleHabitualPdfDownload() {
        ...
        const head = [['Nama Siswa', 'Kelas', 'Tanggal Terlambat', 'Total Keterlambatan Siswa']];
        const body = this.state.habitualResults.map(record => [record.nama, record.kelas, record.tanggal, record.totalCount]);
    ...
    utils: {
        ...
        setDefaultDates() {
            ...
            startDate.setDate(endDate.getDate() - 29); // Set start date to 30 days ago

            // Format to YYYY-MM-DD for the input[type=date]
            const formatDate = (date) => date.toISOString().split('T')[0];

            app.dom.inputs.habitualEndDate.value = formatDate(endDate);
            app.dom.inputs.habitualStartDate.value = formatDate(startDate);
        },
        getChartOptions() {
